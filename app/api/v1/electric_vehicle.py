...
from app.schemas.electric_vehicle import EVform,EVUpdateForm,EVDowntimeRuleCreate,EVDowntimeRuleUpdate,EVDowntimeRuleOut
...
@router_v1.get("/ev/{ev_id}/downtime-rules", response_model=list[EVDowntimeRuleOut])
async def list_blockers(ev_id:int,
                        user:Annotated[UserMe,Depends(get_current_user)],
                        db:Annotated[AsyncSession, Depends(get_async_db)],
                        ev_service:Annotated[EV_service, Depends(create_EV_instance)]):
    return await ev_service.list_downtime_rules(ev_id, user, db)
